...
class MannequinApp(Dash):
    def __init__(self, placements_df: pd.DataFrame):
        ...
        self.x_vals = placements_df['x']
        self.y_vals = placements_df['y']
        self.serial_vals = placements_df['serial_number']
        self.sensor_vals = placements_df['sensor']
    ...
    def update_graph(self, n_clicks, n_intervals):
        ctx = callback_context
        fig = self.layout.app_figure
        ...
        if button_id == 'interval-component':
            ...
            new_z_data = get_data_many(self.sensor_vals, self.serial_vals, 3)
            ...
            # rbfi = Rbf(self.x_vals, self.y_vals, z, function='gaussian')
            # zi = rbfi(xi, yi)
            ...
            fig['data'][0]['text'] = [f"{sn}: {val}" for sn, val in zip(self.serial_vals, z)]
